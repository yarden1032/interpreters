import { partial, map, prop, zipWith } from 'ramda';
import { eqTVar, isAtomicTExp, isProcTExp, isTVar, makeProcTExp, unparseTExp, TExp, TVar, isClassTExp } from "../src/TExp51";
...
export const printSub = (sub: Sub): void => {
    zipWith((v: TVar, te: TExp) => bind(unparseTExp(te), te_s => makeOk(console.log(`${v.var} = ${te_s}`))),
            sub.vars, sub.tes);
}
